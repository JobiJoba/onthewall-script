...
import { getFiles, getStartNumber, upscale_image_square } from "./utils"
import { createMockupSquare, createMockupSquareHomepage } from "./create-mockup"

export async function upscaleSquare() {
  ...
  for (const image of newMockupToProcess) {
    ...
    await createMockupSquareHomepage({
      imagepath: "./process-mockup/square-mockup-homepage.jpg", // the original image
      saveimagepath: `./To-Sell/square-homepage/W-OTWS-${image.number.toString()}.jpg`,
      watermarkpath: "./" + image.fileName,
    })
  }
}
